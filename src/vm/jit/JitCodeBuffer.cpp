...
uint32_t JitCodeBuffer::allocate_executable_memory(std::span<const uint8_t> code) {
  const auto code_alignment = uint64_t(16);
...
void* JitCodeBuffer::get(uint64_t guest_address) const {
...
void* JitCodeBuffer::insert(uint64_t guest_address, std::span<const uint8_t> code) {
...
void* JitCodeBuffer::insert_standalone(std::span<const uint8_t> code) {
  std::unique_lock lock(mutex);
